...
from tensorflow.keras import backend as K, Model, Input, optimizers, initializers
...
def attention(x, mask=None):
    ...
    W = init((x.shape[-1],x.shape[-1]))
    eij = K.tanh(K.dot(x, W))

    ai = K.exp(eij)
    weights = ai/K.sum(ai, axis=1, keepdims=True) # .dimshuffle(0,'x')
    # weights = ai/tf.expand_dims(K.sum(ai, axis=1), axis=-1) # .dimshuffle(0,'x')

    # weighted_input = x*tf.expand_dims(weights, axis=-1) # .dimshuffle(0,1,'x')
    weighted_input = x*weights # .dimshuffle(0,1,'x')
    return weighted_input
    # return K.sum(weighted_input, axis=1, keepdims=True)
...
class TCN(Layer):
    ...
    def call(self, inputs, training=None):
        ...
        self.skip_connections = []
        for layer in self.residual_blocks:
            # self.W = self.init((x.shape[-1], ))
            try:
                # x= attention(x)
                x, skip_out = layer(x, training=training)
                skip_out= attention(skip_out)
                # x= attention(x)
